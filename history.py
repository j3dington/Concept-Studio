...
class HistoryManager:
    ...
    def _apply_patch(self, layer_idx, rect, patch):
        ...
        Enhanced with anti-ghost protection.
        """
        if 0 <= layer_idx < len(self.canvas.layers):
            ...
            # ✅ ANTI-GHOST FIX #1: Clear the target area first with transparency
            painter.setCompositionMode(QPainter.CompositionMode.CompositionMode_Source)
            painter.fillRect(rect, QColor(0, 0, 0, 0))  # Transparent black
            
            # ✅ ANTI-GHOST FIX #2: Apply patch with Source mode (complete replacement)
            painter.setCompositionMode(QPainter.CompositionMode.CompositionMode_Source)
            painter.drawPixmap(rect.topLeft(), patch)
            painter.end()
            
            # ✅ This now calls the improved compose_layers() with repaint()
